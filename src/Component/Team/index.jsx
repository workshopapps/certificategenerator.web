import React from "react";

import "./team.style.scss";
import Anees from "./images/anees.svg";
import Icon1 from "./images/icon_1.svg";
import Icon2 from "./images/icon_2.svg";
import Icon3 from "./images/icon_3.svg";

const Team = () => {
  return (
    <section>
      {/* title */}
      <div className="team">Our Team</div>
      <div className="meet">Meet our innovative and creative team behind the vision of our software</div>
      {/* members */}
      <div className="container">
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
        <figure>
          <img src={Anees} alt="" />
          <figcaption className="name">Anees Adeyinka</figcaption>
          <figcaption className="title">Product designer</figcaption>
          <div className="icon">
            <img src={Icon1} alt="" height='19.94px' width='19.96px' />
            <img src={Icon2} alt="" height='19.94px' width='19.96px' />
            <img src={Icon3} alt="" height='19.94px' width='19.96px' />
          </div>
        </figure>
      </div>
      {/* button */}
      <button></button>
    </section>
  );
};

export default Team;
