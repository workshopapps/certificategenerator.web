const fontToBase64 = require("node-font2base64");
const path = require("path");

const fontData = fontToBase64.encodeToDataSrcSync(
  path.resolve(__dirname, "fonts", "Georgia.ttf")
);
...
function getSingleTemplate(templateId = 1) {
  let id = templateId;

  // if templateId is invalid (less than or equal to zero or greater than the number of templates) set id to 1
  if (templateId <= 0 || templateId > Object.keys(templates).length) id = 1;

  return templateSingle[id];
}
