...
const UploadCSV = () => {
  ...
  const toggleState = (e) => {
   ...
   const active =  Object.values(e.target.classList).find(element => element === 'active')
    //   .forEach(element => {
      if (!active) {
        // console.log(3);
        setState(prev => {
  ...
  return (
      ...
      <div className='certificateSwitch'>
        <button className={!state.active ? 'active switchBtn' : 'switchBtn'} onClick={toggleState}> Single Certificate</button>
        <button className={state.active ? 'active switchBtn' : 'switchBtn'} onClick={toggleState} > Bulk</button>
      ...
      <Button name={'Modify Certificate'} className={'modiftyCertificate'} />
          ...
          <div><img src={CSVSample} alt="CSV sample" /></div>
        ...
        <span>Drag and drop your CSV file here<br /> or
